...
class NeRFDataset:
    def __init__(self, opt, device, type='train', downscale=1, n_test=50):
        ...
        # resolve dataset include folder
        ...
        if self.mode == 'colmap' and type == 'test':
            ...
            if self.opt.camerapos[0] == 360 and self.opt.camerapos[1] == 360 and self.opt.camerapos[2] == 360:
                ...
                pose0[:3, :3] = r
                pose0[0, 3] = self.opt.camerapos[3]
                pose0[1, 3] = self.opt.camerapos[4]
                pose0[2, 3] = self.opt.camerapos[5]
            ...
            for i in range(0, 360, int(360/n_test)):
                if self.view == 'random':
                    ...
                    pose_center[:3, :3] = Rotation.from_euler('y', [i], degrees=True).as_matrix()
                    pose = np.matmul(pose_center, pose0)
                elif self.view == 'pitch':
                    pose_center[:3, :3] = Rotation.from_euler('x', [i], degrees=True).as_matrix()
                    pose = np.matmul(pose_center, pose0)
                elif self.view == 'roll':
